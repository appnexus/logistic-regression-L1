...
class LogisticRegressionL1():
    ...
    def __calc_new_weights(self, matrix, betas):
        ...
        Params
        ------
        matrix : ndarray with bias, X, weights (num trials), and responses
        betas : array of coefficients, includes bias
    ...
    def fit(self, matrix, lambda_grid=np.exp(-1*np.linspace(1,17,300)),
        ...
        Returns
        -------
        beta_path : ndarray, betas for each lambda iteration
        """
        if pyspark == False:
            # Add in bias
    ...
    def predict(self, X, pyspark=False):
        ...
        Params
        ------
        X : ndarray, hold-out data of shape [n_samples, n_features]
        pyspark : boolean

        note: I didn't put weights here because I'm predicting a probability.
        With weights, you can predict the number of "successes."

        Returns
        -------
        C: array, shape [n_samples] of probabilities between 0, 1
        """
        if pyspark == False:
            # Add in bias
            ...
            return X.map(lambda x: self.__calc_prob(x, self.coef_))
